...
// LiveComponentInfo holds the output of TranspileLive.
type LiveComponentInfo struct {
	PackageName string // e.g. "counter"
	SourceCode  string // complete Go source implementing livepage.Page
	Route       string // derived route pattern, e.g. "/counter"
}

// TranspileLive converts a .live.gox File into a Go source file that
// implements livepage.Page. The generated file is compiled by the host
// Go toolchain (not TinyGo) and registered with the live page registry.
//
// Generated structure:
//
//	type <Name>State struct { ... }   // from frontmatter var declarations
//	type <Name>Page struct{}
//	func (*<Name>Page) Init(...)      // returns zero-value state
//	func (*<Name>Page) Render(...)    // HTML template with {var} interpolation
//	func (*<Name>Page) Handle(...)    // dispatches evt.Name to handler funcs
func TranspileLive(f *File) *LiveComponentInfo {
	name := livePageName(f.Path)
	pkg := f.PackageName()
	...
	sb.WriteString("\tpubliclive \"github.com/dev2k6/Nguyen.go/pkg/live\"\n")
	sb.WriteString(")\n\n")

	// Suppress unused import warnings for fmt/strings if not needed —
	// we emit a compile-time blank usage at the bottom.
	sb.WriteString("var _ = fmt.Sprintf\n")
	sb.WriteString("var _ = strings.TrimSpace\n\n")

	// State struct — collect var declarations from frontmatter
	stateFields := extractLiveStateFields(f.GoCode)
	sb.WriteString("// " + name + "State holds per-session state for the " + name + " live page.\n")
	sb.WriteString("type " + name + "State struct {\n")
	...
	// Page type
	sb.WriteString("// " + name + "Page implements livepage.Page.\n")
	sb.WriteString("type " + name + "Page struct{}\n\n")

	// User-defined handler functions from frontmatter
	...
	// Init
	sb.WriteString("func (p *" + name + "Page) Init(_ context.Context, _ map[string]string) (any, error) {\n")
	sb.WriteString("\treturn &" + name + "State{}, nil\n")
	sb.WriteString("}\n\n")

	// Render — replace {var} with fmt.Sprintf against typed state
	sb.WriteString("func (p *" + name + "Page) Render(_ context.Context, rawState any) (string, error) {\n")
	sb.WriteString("\tstate, _ := rawState.(*" + name + "State)\n")
	sb.WriteString("\tif state == nil {\n")
	sb.WriteString("\t\tstate = &" + name + "State{}\n")
	sb.WriteString("\t}\n")
	renderExpr := buildLiveRenderExpr(f.HTMLTemplate, stateFields)
	sb.WriteString("\treturn " + renderExpr + ", nil\n")
	sb.WriteString("}\n\n")

	// Handle — dispatch on evt.Name
	handlers := extractEventHandlers(f.HTMLTemplate)
	sb.WriteString("func (p *" + name + "Page) Handle(_ context.Context, rawState any, evt publiclive.Event) (any, error) {\n")
	sb.WriteString("\tstate, _ := rawState.(*" + name + "State)\n")
	sb.WriteString("\tif state == nil {\n")
	sb.WriteString("\t\tstate = &" + name + "State{}\n")
	sb.WriteString("\t}\n")
	if len(handlers) > 0 {
		sb.WriteString("\tswitch evt.Name {\n")
	...
	sb.WriteString("\treturn state, nil\n")
	sb.WriteString("}\n")

	route := liveRouteFromPath(f.Path)
...
// livePageName derives a CamelCase page name from the file path.
// "pages/counter.live.gox" → "Counter"
// "pages/admin/dashboard.live.gox" → "Dashboard"
func livePageName(path string) string {
	base := filepath.Base(path)
	base = strings.TrimSuffix(base, ".live.gox")
...
// liveRouteFromPath derives the URL route from the file path.
// "pages/counter.live.gox" → "/counter"
// "pages/index.live.gox"   → "/"
func liveRouteFromPath(path string) string {
	base := filepath.Base(path)
	base = strings.TrimSuffix(base, ".live.gox")
	base = strings.TrimSuffix(base, ".gox")
	if base == "index" {
...
// extractLiveStateFields parses `var fieldName Type` declarations from
// frontmatter and returns them as struct field strings.
func extractLiveStateFields(goCode string) []string {
	var fields []string
	varRx := regexp.MustCompile(`(?m)^var\s+(\w+)\s+(\w[\w\.\[\]]*)\s*$`)
	matches := varRx.FindAllStringSubmatch(goCode, -1)
	for _, m := range matches {
		fields = append(fields, m[1]+" "+m[2])
	}
	// Also handle var name = initialValue — extract name and infer type as any
	varInitRx := regexp.MustCompile(`(?m)^var\s+(\w+)\s*=\s*(.+)$`)
	initMatches := varInitRx.FindAllStringSubmatch(goCode, -1)
	...
	for _, m := range initMatches {
		if !seen[m[1]] {
			fields = append(fields, m[1]+" any")
		}
	}
	return fields
}

// buildLiveRenderExpr builds a Go expression that produces the rendered HTML
// string by replacing {varName} placeholders with state field accesses.
func buildLiveRenderExpr(tmpl string, stateFields []string) string {
	// Build set of known state field names
	fieldNames := make(map[string]bool)
	for _, f := range stateFields {
		parts := strings.Fields(f)
		if len(parts) > 0 {
			fieldNames[parts[0]] = true
		}
	}

	varRx2 := regexp.MustCompile(`\{([a-zA-Z_][a-zA-Z0-9_]*)\}`)
	vars := varRx2.FindAllStringSubmatch(tmpl, -1)
	if len(vars) == 0 {
		return fmt.Sprintf("%q", tmpl)
	}

	// Build fmt.Sprintf call
	fmtStr := varRx2.ReplaceAllStringFunc(tmpl, func(m string) string {
		return "%v"
	})
	var args []string
	for _, v := range vars {
